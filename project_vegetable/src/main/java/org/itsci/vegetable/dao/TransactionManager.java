...
public class TransactionManager {
	...
	public List<Assets> getAssets() {
		...
		try {
			...
			while (rs.next()) {
				...
				Assets a = new Assets(asset_id, asset_price, equipment_name, equipment_unit, product_name, product_unit,
						p);
				as.add(a);
			}
			con.close();
	...
	public List<Transaction> list_transaction() {
		...
		try {
			...
			while (rs.next()) {
				...
				tdate.set(Integer.parseInt(date[0]), Integer.parseInt(date[1]) - 1, Integer.parseInt(date2[0]),
						Integer.parseInt(date3[0]), Integer.parseInt(date3[1]));
	...
	public Transaction_details transaction_details_id(int tdid, int tid) {
		...
		try {
			Statement stmt = con.createStatement();
			String sql = "select * from transaction_details where transaction_detail_id = " + tdid
	 ...
	 * 
	 * Transaction_details t = new
	 * Transaction_details(transaction_detail_id,type,amount,sum,ts,as);
	 * tsd.add(t);
	 * 
	 * }
	 * con.close();
		...
		try {
			...
			if (t.equals("1") && !d.equals("")) {
				sql = "select ANY_VALUE(td.transaction_detail_id),ANY_VALUE(td.amount),ANY_VALUE(td.sum),ANY_VALUE(td.type),ANY_VALUE(td.asset_id),ANY_VALUE(td.transaction_id),sum(td.sum) "
						...
						+ "where CAST(t.date_time AS date) = '" + d + "' and year(t.date_time) = '" + year
						+ "' and t.term = " + term + " group by CAST(t.date_time AS date);";
			} else if (!t.equals("1") && d.equals("")) {
				sql = "select ANY_VALUE(td.transaction_detail_id),ANY_VALUE(td.amount),ANY_VALUE(td.sum),ANY_VALUE(td.type),ANY_VALUE(td.asset_id),ANY_VALUE(td.transaction_id),sum(td.sum) "
						+ " from transaction_details td inner join transaction t on t.transaction_id = td.transaction_id   "
						+ "where td.type = '" + t + "' and year(t.date_time) = '" + year + "' and t.term = " + term
						+ " group by CAST(t.date_time AS date);";
			} else if (t.equals("1") && d.equals("")) {
				sql = "select ANY_VALUE(td.transaction_detail_id),ANY_VALUE(td.amount),ANY_VALUE(td.sum),ANY_VALUE(td.type),ANY_VALUE(td.asset_id),ANY_VALUE(td.transaction_id),sum(td.sum) "
						+ " from transaction_details td inner join transaction t on t.transaction_id = td.transaction_id   "
						+ "where year(t.date_time) = '" + year + "' and t.term = " + term
						+ " group by CAST(t.date_time AS date);";
			} else {
				sql = "select ANY_VALUE(td.transaction_detail_id),ANY_VALUE(td.amount),ANY_VALUE(td.sum),ANY_VALUE(td.type),ANY_VALUE(td.asset_id),ANY_VALUE(td.transaction_id),sum(td.sum) "
						+ " from transaction_details td inner join transaction t on t.transaction_id = td.transaction_id   "
						+ "where CAST(t.date_time AS date) = '" + d + "' and year(t.date_time) = '" + year
						+ "' and t.term = " + term + " and td.type = '" + t + "' group by CAST(t.date_time AS date);";
			}
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
				int transaction_detail_id = rs.getInt(1);
	...
	public List<Transaction_details> Alltransaction_by_search(String t, String y) {
		...
		try {
			Statement stmt = con.createStatement();
			String sql = "select ANY_VALUE(td.transaction_detail_id),ANY_VALUE(td.amount),ANY_VALUE(td.sum),ANY_VALUE(td.type),ANY_VALUE(td.asset_id),ANY_VALUE(td.transaction_id),sum(td.sum)"
					+ " from transaction_details td inner join transaction t on t.transaction_id = td.transaction_id   "
	...
	public List<Transaction_details> transaction_details_by_date(String t, String d) {
		...
		try {
			...
			String sql = "select td.transaction_detail_id,td.amount,td.sum,td.type,td.asset_id,td.transaction_id,t.date_time  from transaction_details td "
					+ "inner join transaction t on t.transaction_id = td.transaction_id where CAST(t.date_time AS date) = '"
					+ d + "' and td.type='" + t + "'";
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
				int transaction_detail_id = rs.getInt(1);
